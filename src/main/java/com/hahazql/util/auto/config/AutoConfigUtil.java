...
import com.hahazql.util.auto.config.data.ClassConfigData;
import com.hahazql.util.auto.config.data.FieldConfigData;
import com.hahazql.util.auto.config.data.PackageConfigData;
import com.hahazql.util.clazz.ClassLoadUtil;
import com.hahazql.util.clazz.ClassUtil;
import com.hahazql.util.clazz.PackageUtil;
import com.hahazql.util.clazz.TypeUtils;
import com.alibaba.fastjson.parser.ParserConfig;
import com.hahazql.util.exception.BaseException;
import com.hahazql.util.helper.ArrayUtil;
...
public class AutoConfigUtil
{
     ...
     * 允许的属性类型
     */
    public static final Class[] fieldTypeList = new Class[] {java.lang.String.class,java.lang.Integer.class,
            java.lang.Long.class,java.lang.Boolean.class,java.lang.Float.class,
     ...
     * @throws IllegalAccessException
     */
    public static <T> void createXMLElement(AutoConfig config, T[] list, Element rootElement) throws InvocationTargetException, IllegalAccessException {
        for(T t : list)
        {
            ...
            for(Object _fieldConfig : config.getFieldList())
            {
                ...
                Element fieldElement = element.addElement(fieldConfig.getName());
                fieldElement.addAttribute("name",fieldConfig.getFieldName());
                fieldElement.addAttribute("desc",fieldConfig.getFieldDesc());
                if(!fieldConfig.isArray())//非数组情况直接使用
                {
                    fieldElement.setText(ClassUtil.getFieldValue(t, fieldConfig.getName(), fieldConfig.getFieldType()).toString());
                }
                else
                {
                    Object[] elementList = (Object[])ClassUtil.getFieldValue(t,fieldConfig.getName(),fieldConfig.getFieldType());
                    createXMLElement(fieldConfig.getFieldConfig(), elementList, fieldElement);
                }
            }
        }
    }
     ...
     * @throws InvocationTargetException
     */
    public static <T> String createXML(String classPath, List<T> configList) throws ClassNotFoundException, NoSuchMethodException, InstantiationException, IllegalAccessException, InvocationTargetException {
        ...
        AutoConfigUtil.createXMLElement(config, configList.toArray(), rootElement);
        return document.asXML();
    }

    public static <T> void createXML(AutoConfig config, List<T> configList,String xmlPath) throws ClassNotFoundException, NoSuchMethodException, InstantiationException, IllegalAccessException, InvocationTargetException, IOException {
        Document document = DocumentHelper.createDocument();
     ...
     * @throws InvocationTargetException
     */
    public static <T> T readXMLElement(AutoConfig config,Class<T> clazz,Element rootElement) throws IllegalAccessException, InstantiationException, BaseException, InvocationTargetException {
            ...
            if(fieldConfig.isArray())//若属性为数组
            {
                ...
                for (Iterator _it = _elements.iterator(); _it.hasNext()&&_elements.size()>0;)
                {
                    ...
                    if(_element.getName() != fieldConfig.getFieldConfig().getConfigName())//只获取符合标准的节点
                        continue;
                    Array.set(array,nowIndex,readXMLElement(fieldConfig.getFieldConfig(),fieldConfig.getElementClazz(),_element));
                }
     ...
     */
    public static <T> List<T> readXml(Class<T> clazz,String xml) throws ClassNotFoundException, NoSuchMethodException, InstantiationException, IllegalAccessException, InvocationTargetException, BaseException, DocumentException {
        ...
        for (Iterator it = elements.iterator(); it.hasNext();) {
            Element element = (Element) it.next();
            T t = readXMLElement(config,clazz,element);
            list.add(t);
        }
        return list;
    }

    public static <T> List<T> readXml(AutoConfig config,Class<T> clazz,String xml) throws ClassNotFoundException, NoSuchMethodException, InstantiationException, IllegalAccessException, InvocationTargetException, BaseException, DocumentException {
        Document document = DocumentHelper.parseText(xml);
        Element rootElement = document.getRootElement();
        List<T> list = new ArrayList<T>();
        List elements = rootElement.elements();
     ...
     * @throws BaseException
     */
    public static <T> List<T> readExcel(AutoConfig<AutoExcel> autoConfig,Class<T> clazz,String excelPath,String indexSheetName,int startRow,int endRow) throws IOException, IllegalAccessException, InstantiationException, InvocationTargetException, BaseException {
